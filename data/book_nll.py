# %% [markdown]
# ### Process book data and calculate NLL (Negative Log Likelihood)
...
def process_book_data():
    """
    Process all JSON files in the book folder and calculate NLLs
    """
    folder_path = './book/'
    assert os.path.exists(folder_path), f"Folder {folder_path} does not exist"
    
    # Get all JSON files in the book folder
    json_files = [f for f in os.listdir(folder_path) if f.endswith('.json')]
    print(f"Found {len(json_files)} JSON files in book folder")
